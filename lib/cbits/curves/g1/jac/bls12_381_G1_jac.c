...
void bls12_381_G1_jac_batch_from_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bls12_381_G1_jac_from_affine(p,q);
    p += 2*NLIMBS_P;
    q += 3*NLIMBS_P;
  }
}
...
void bls12_381_G1_jac_batch_to_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bls12_381_G1_jac_to_affine(p,q);
    p += 3*NLIMBS_P;
    q += 2*NLIMBS_P;
  }
}
...
//  - projective (jac) Montgomery point
//
void bls12_381_G1_jac_MSM_binary_coeff_jac_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  bls12_381_G1_jac_set_infinity( tgt );
  for(int i=0; i<npoints; i++) { 
    if (expos[i]) {
      // add to the running sum
      bls12_381_G1_jac_madd_inplace( tgt , grps  + i*2*NLIMBS_P );
    }
  }
}

void bls12_381_G1_jac_MSM_binary_coeff_affine_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  uint64_t tmp[3*NLIMBS_P];
  bls12_381_G1_jac_MSM_binary_coeff_jac_out(npoints, expos, grps, tmp);
  bls12_381_G1_jac_to_affine(tmp, tgt);
}
...
//
void bls12_381_G1_jac_multi_MSM_binary_coeff_jac_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  ...
  for(int i=0; i<nrows; i++) { 
    bls12_381_G1_jac_set_infinity( tgt + i*3*NLIMBS_P );
  }
  
  uint64_t *precalc = (uint64_t*) malloc( (1<<c) * 8*3*NLIMBS_P );
  assert( precalc != 0 );
  bls12_381_G1_jac_set_infinity( precalc );
  ...
  for(int k=0; k<nwindows; k++) {
    ...
    for(int j=0; j<C; j++) {
      if (j==0) {
        bls12_381_G1_jac_from_affine( gptr , PRECALC(1) );
      }
      else {
        int m = (1<<j);
        for(int i=0; i<m; i++) {
          bls12_381_G1_jac_madd_jac_aff( PRECALC(i) , gptr , PRECALC(i+m) );
        }
      }
      gptr += 2*NLIMBS_P;
    }
    ...
    for(int i=0; i<nrows; i++) {
      ...
      if (idx > 0) {
        // add to the running sum
        bls12_381_G1_jac_add_inplace( tgt + i*3*NLIMBS_P , PRECALC(idx) );
      }
    }
...
void bls12_381_G1_jac_multi_MSM_binary_coeff_affine_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  uint64_t *tmp = (uint64_t*) malloc( nrows * 8*3*NLIMBS_P );
  assert( tmp != 0 );
  bls12_381_G1_jac_multi_MSM_binary_coeff_jac_out( nrows, npoints, coeffs, grps, tmp);
  for(int i=0; i<nrows; i++) {
    bls12_381_G1_jac_to_affine( tmp + i*3*NLIMBS_P, tgt + i*2*NLIMBS_P );
  }
  free(tmp);
}
