...
void bls12_381_G1_proj_scale_by_3B(const uint64_t *src, uint64_t *tgt ) {
  ...
  bls12_381_Fp_mont_add( src, src, tmp );       // 2*B
  bls12_381_Fp_mont_add_inplace( tmp, tmp );    // 4*B
  bls12_381_Fp_mont_add( tmp, tmp, tmp2);       // 8*B
  bls12_381_Fp_mont_add( tmp, tmp2, tgt );      // 12*B
}
...
void bls12_381_G1_proj_normalize( const uint64_t *src1, uint64_t *tgt ) {
  ...
  if (bls12_381_Fp_mont_is_zero( Z1 ) ) {
    ...
    if (bls12_381_Fp_mont_is_one( Z1 )) {
      ...
      bls12_381_Fp_mont_mul( X1, zinv, X3 );
      bls12_381_Fp_mont_mul( Y1, zinv, Y3 );
...
void bls12_381_G1_proj_batch_from_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bls12_381_G1_proj_from_affine(p,q);
    p += 2*NLIMBS_P;
    q += 3*NLIMBS_P;
  }
}
...
void bls12_381_G1_proj_batch_to_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bls12_381_G1_proj_to_affine(p,q);
    p += 3*NLIMBS_P;
    q += 2*NLIMBS_P;
  }
}
...
uint8_t bls12_381_G1_proj_is_on_curve ( const uint64_t *src1 ) {
  ...
  bls12_381_Fp_mont_mul_inplace( acc, Z1 );     // Y^2*Z
  bls12_381_Fp_mont_neg_inplace( acc );         // -Y^2*Z
  bls12_381_Fp_mont_sqr( X1, tmp );             // X^2
  bls12_381_Fp_mont_mul_inplace( tmp, X1 );     // X^3
  bls12_381_Fp_mont_add_inplace( acc, tmp );    // - Y^2*Z + X^3
  bls12_381_Fp_mont_sqr( Z1, ZZ );              // Z^2
  bls12_381_Fp_mont_mul( Z1, ZZ, tmp );          // Z^3
  bls12_381_G1_proj_scale_by_B_inplace( tmp );   // B*Z^3
  bls12_381_Fp_mont_add_inplace( acc, tmp );     // - Y^2*Z + X^3 + A*X*Z^2 + B*Z^3
...
uint8_t bls12_381_G1_proj_is_in_subgroup ( const uint64_t *src1 ) {
  ...
  if (!bls12_381_G1_proj_is_on_curve(src1)) {
    ...
    bls12_381_G1_proj_scl_Fr_std( bls12_381_G1_proj_cofactor , src1 , tmp );
...
void bls12_381_G1_proj_dbl( const uint64_t *src1, uint64_t *tgt ) {
  ...
  bls12_381_Fp_mont_add( XX, XX, w );       // w  = 2*XX
  bls12_381_Fp_mont_add_inplace( w, XX );   // w  = 3*XX
  bls12_381_Fp_mont_mul( Y1 , Z1 , s );         // s   = Y1*Z1
  bls12_381_Fp_mont_add_inplace( s, s );        // s   = 2*Y1*Z1
  bls12_381_Fp_mont_sqr( s , ss );              // ss  = s^2
  bls12_381_Fp_mont_mul( Y1 , s , R);           // R   = Y1*s
  bls12_381_Fp_mont_sqr( R , RR );              // RR  = R^2
  bls12_381_Fp_mont_add( X1 , R , B);           // B   = (X1+R)
  bls12_381_Fp_mont_sqr_inplace( B );           // B   = (X1+R)^2
  bls12_381_Fp_mont_sub_inplace( B , XX );      // B   = (X1+R)^2 - XX
  bls12_381_Fp_mont_sub_inplace( B , RR );      // B   = (X1+R)^2 - XX - RR
  bls12_381_Fp_mont_sqr( w , h );               // h   = w^2
  bls12_381_Fp_mont_sub_inplace( h , B );       // h   = w^2 - B
  bls12_381_Fp_mont_sub_inplace( h , B );       // h   = w^2 - 2*B
  bls12_381_Fp_mont_mul( s , ss , Z3 );         // Z3  = s^3
  bls12_381_Fp_mont_mul( h , s , X3 );          // X3  = h*s
  bls12_381_Fp_mont_sub( B , h , Y3 );          // Y3  = B-h
  bls12_381_Fp_mont_mul_inplace( Y3 , w  );     // Y3  = w*(B-h)
  bls12_381_Fp_mont_sub_inplace( Y3 , RR );     // Y3  = w*(B-h) - RR
  bls12_381_Fp_mont_sub_inplace( Y3 , RR );     // Y3  = w*(B-h) - 2*RR
}
...
void bls12_381_G1_proj_add( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  ...
  uint64_t t4[6];
  uint64_t t5[6];
  bls12_381_Fp_mont_mul( X1, X2, t0 );              // t0 = X1*X2
  bls12_381_Fp_mont_mul( Y1, Y2, t1 );              // t1 = Y1*Y2
  bls12_381_Fp_mont_mul( Z1, Z2, t2 );              // t2 = Z1*Z2
  bls12_381_Fp_mont_add( X1, Y1, t3 );              // t3 = X1+Y1
  bls12_381_Fp_mont_add( X2, Y2, t4 );              // t4 = X2+Y2
  bls12_381_Fp_mont_mul_inplace( t3, t4 );          // t3 = t3*t4
  bls12_381_Fp_mont_add( t0, t1, t4 );              // t4 = t0+t1
  bls12_381_Fp_mont_sub_inplace( t3 , t4 );         // t3 = t3-t4
  bls12_381_Fp_mont_add( X1, Z1, t4 );              // t4 = X1+Z1
  bls12_381_Fp_mont_add( X2, Z2, t5 );              // t5 = X2+Z2
  bls12_381_Fp_mont_mul_inplace( t4, t5 );          // t4 = t4*t5
  bls12_381_Fp_mont_add( t0, t2, t5 );              // t5 = t0+t2
  bls12_381_Fp_mont_sub_inplace( t4, t5 );          // t4 = t4-t5
  bls12_381_Fp_mont_add( Y1, Z1, t5 );              // t5 = Y1+Z1
  bls12_381_Fp_mont_add( Y2, Z2, X3 );              // X3 = Y2+Z2
  bls12_381_Fp_mont_mul_inplace( t5, X3 );          // t5 = t5*X3
  bls12_381_Fp_mont_add( t1, t2, X3 );              // X3 = t1+t2
  bls12_381_Fp_mont_sub_inplace( t5, X3 );          // t5 = t5-X3
  bls12_381_G1_proj_scale_by_3B( t2, X3 );          // X3 = b3*t2
  bls12_381_Fp_mont_copy( X3, Z3 );                 // Z3 = X3
  bls12_381_Fp_mont_sub( t1, Z3, X3 );              // X3 = t1-Z3
  bls12_381_Fp_mont_add_inplace( Z3, t1 );          // Z3 = t1+Z3
  bls12_381_Fp_mont_mul( X3, Z3, Y3 );              // Y3 = X3*Z3
  bls12_381_Fp_mont_add( t0, t0, t1 );              // t1 = t0+t0
  bls12_381_Fp_mont_add_inplace( t1, t0 );          // t1 = t1+t0
  bls12_381_G1_proj_scale_by_3B_inplace( t4 );      // t4 = b3*t4
  bls12_381_Fp_mont_mul( t1, t4, t0 );              // t0 = t1*t4
  bls12_381_Fp_mont_add_inplace( Y3, t0 );          // Y3 = Y3+t0
  bls12_381_Fp_mont_mul( t4, t5, t0 );              // t0 = t5*t4
  bls12_381_Fp_mont_mul_inplace( X3, t3 );          // X3 = t3*X3
  bls12_381_Fp_mont_sub_inplace( X3, t0 );          // X3 = X3-t0
  bls12_381_Fp_mont_mul( t1, t3, t0 );              // t0 = t3*t1
  bls12_381_Fp_mont_mul_inplace( Z3, t5 );          // Z3 = t5*Z3
  bls12_381_Fp_mont_add_inplace( Z3, t0 );          // Z3 = Z3+t0
}
...
void bls12_381_G1_proj_madd_proj_aff( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  ...
  uint64_t   R[6];
  uint64_t   A[6];
  bls12_381_Fp_mont_mul( Y2, Z1, u );              //  u = Y2*Z1     
  bls12_381_Fp_mont_sub_inplace( u , Y1 );         //  u = Y2*Z1-Y1  
  bls12_381_Fp_mont_mul( X2, Z1, v );              //  v = X2*Z1     
  bls12_381_Fp_mont_sub_inplace( v , X1 );         //  v = X2*Z1-X1  
  if (bls12_381_Fp_mont_is_zero(u) && bls12_381_Fp_mont_is_zero(v) ) {
    // it's doubling, the naive result would be (0,0,0)
    bls12_381_G1_proj_dbl( src1 , tgt );
    return;
  }
  bls12_381_Fp_mont_sqr( u , uu );                 //  uu = u2       
  bls12_381_Fp_mont_sqr( v, vv );                  //  vv = v2       
  bls12_381_Fp_mont_mul( v, vv, vvv );             //  vvv = v*vv    
  bls12_381_Fp_mont_mul( vv, X1, R );              //  R = vv*X1     
  bls12_381_Fp_mont_mul( uu, Z1, A );              //  A = uu*Z1     
  bls12_381_Fp_mont_sub_inplace( A, vvv );         //  A = uu*Z1-vvv 
  bls12_381_Fp_mont_sub_inplace( A, R );           //  A = uu*Z1-vvv-2  
  bls12_381_Fp_mont_sub_inplace( A, R );           //  A = uu*Z1-vvv-2*R
  bls12_381_Fp_mont_mul( v, A, X3 );               //  X3 = v*A      
  bls12_381_Fp_mont_mul( Z1, vvv, Z3 );            //  Z3 = vvv*Z1   
  bls12_381_Fp_mont_sub_inplace( R , A );          //  R' =  R-A     
  bls12_381_Fp_mont_mul_inplace( vvv , Y1 );       //  vvv' = vvv*Y1 
  bls12_381_Fp_mont_mul( u , R , Y3 );             //  Y3 = u*(R-A)  
  bls12_381_Fp_mont_sub_inplace( Y3, vvv );        //  Y3 = u*(R-A)-vvv*Y1
}
...
void bls12_381_G1_proj_scl_Fr_mont(const uint64_t *expo, const uint64_t *grp, uint64_t *tgt) {
  ...
  bls12_381_G1_proj_scl_generic(expo_std, grp, tgt, NLIMBS_R);
}
...
//  - projective (proj) Montgomery point
//
void bls12_381_G1_proj_MSM_binary_coeff_proj_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  bls12_381_G1_proj_set_infinity( tgt );
  for(int i=0; i<npoints; i++) { 
    if (expos[i]) {
      // add to the running sum
      bls12_381_G1_proj_madd_inplace( tgt , grps  + i*2*NLIMBS_P );
    }
  }
}

void bls12_381_G1_proj_MSM_binary_coeff_affine_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  uint64_t tmp[3*NLIMBS_P];
  bls12_381_G1_proj_MSM_binary_coeff_proj_out(npoints, expos, grps, tmp);
  bls12_381_G1_proj_to_affine(tmp, tgt);
}
...
//
void bls12_381_G1_proj_multi_MSM_binary_coeff_proj_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  ...
  for(int i=0; i<nrows; i++) { 
    bls12_381_G1_proj_set_infinity( tgt + i*3*NLIMBS_P );
  }
  
  uint64_t *precalc = (uint64_t*) malloc( (1<<c) * 8*3*NLIMBS_P );
  assert( precalc != 0 );
  bls12_381_G1_proj_set_infinity( precalc );
  ...
  for(int k=0; k<nwindows; k++) {
    ...
    for(int j=0; j<C; j++) {
      if (j==0) {
        bls12_381_G1_proj_from_affine( gptr , PRECALC(1) );
      }
      else {
        int m = (1<<j);
        for(int i=0; i<m; i++) {
          bls12_381_G1_proj_madd_proj_aff( PRECALC(i) , gptr , PRECALC(i+m) );
        }
      }
      gptr += 2*NLIMBS_P;
    }
    ...
    for(int i=0; i<nrows; i++) {
      ...
      if (idx > 0) {
        // add to the running sum
        bls12_381_G1_proj_add_inplace( tgt + i*3*NLIMBS_P , PRECALC(idx) );
      }
    }
...
void bls12_381_G1_proj_multi_MSM_binary_coeff_affine_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  uint64_t *tmp = (uint64_t*) malloc( nrows * 8*3*NLIMBS_P );
  assert( tmp != 0 );
  bls12_381_G1_proj_multi_MSM_binary_coeff_proj_out( nrows, npoints, coeffs, grps, tmp);
  for(int i=0; i<nrows; i++) {
    bls12_381_G1_proj_to_affine( tmp + i*3*NLIMBS_P, tgt + i*2*NLIMBS_P );
  }
  free(tmp);
}
...
void bls12_381_G1_proj_fft_forward_noalloc( int m, int src_stride, const uint64_t *gen, const uint64_t *src, uint64_t *buf, uint64_t *tgt) {

  if (m==0) {
    bls12_381_G1_proj_copy( src, tgt );
    return;
  }

  if (m==1) {
    // N = 2
    bls12_381_G1_proj_add( src , src + src_stride*GRP_NLIMBS , tgt              );    // x + y
    bls12_381_G1_proj_sub( src , src + src_stride*GRP_NLIMBS , tgt + GRP_NLIMBS );    // x - y
    return;
  }
    ...
    bls12_381_G1_proj_fft_forward_noalloc( m-1 , src_stride<<1 , gpow , src                         , buf + N*GRP_NLIMBS , buf                    );
    bls12_381_G1_proj_fft_forward_noalloc( m-1 , src_stride<<1 , gpow , src + src_stride*GRP_NLIMBS , buf + N*GRP_NLIMBS , buf + halfN*GRP_NLIMBS );

    bls12_381_Fr_mont_set_one(gpow);
    for(int j=0; j<halfN; j++) {
      bls12_381_G1_proj_scl_Fr_mont(gpow , buf + (j+halfN)*GRP_NLIMBS  , tgt +  j *GRP_NLIMBS );  //   g*v[k]
      bls12_381_G1_proj_neg ( tgt +  j       *GRP_NLIMBS ,        tgt + (j+halfN)*GRP_NLIMBS );   // - g*v[k]
      bls12_381_G1_proj_add_inplace( tgt +  j       *GRP_NLIMBS , buf + j*GRP_NLIMBS );           // u[k] + g*v[k]
      bls12_381_G1_proj_add_inplace( tgt + (j+halfN)*GRP_NLIMBS , buf + j*GRP_NLIMBS );           // u[k] - g*v[k]
...
void bls12_381_G1_proj_fft_forward (int m, const uint64_t *gen, const uint64_t *src, uint64_t *tgt) {
  int N = (1<<m);
  uint64_t *buf = malloc( 8*GRP_NLIMBS * (2*N) );
  assert( buf !=0 );
  bls12_381_G1_proj_fft_forward_noalloc( m, 1, gen, src, buf, tgt);
  free(buf);
  for(int i=0; i<N; i++) { 
    bls12_381_G1_proj_normalize_inplace( tgt + i*GRP_NLIMBS );
  }
}
...
const uint64_t bls12_381_G1_proj_oneHalf[4] = { 0x00000000ffffffff, 0xac425bfd0001a401, 0xccc627f7f65e27fa, 0x0c1258acd66282b7 };

void bls12_381_G1_proj_fft_inverse_noalloc(int m, int tgt_stride, const uint64_t *gen, const uint64_t *src, uint64_t *buf, uint64_t *tgt) {
  ...
  if (m==1) {
    // N = 2
    bls12_381_G1_proj_add( src , src + GRP_NLIMBS , tgt                         );   // x + y
    bls12_381_G1_proj_sub( src , src + GRP_NLIMBS , tgt + tgt_stride*GRP_NLIMBS );   // x - y
    bls12_381_G1_proj_scl_Fr_mont( bls12_381_G1_proj_oneHalf , tgt                         , tgt                         );      // (x + y)/2
    bls12_381_G1_proj_scl_Fr_mont( bls12_381_G1_proj_oneHalf , tgt + tgt_stride*GRP_NLIMBS , tgt + tgt_stride*GRP_NLIMBS );      // (x - y)/2
    return;
  }
    ...
    uint64_t gpow[NLIMBS_R];    
    bls12_381_Fr_mont_copy(bls12_381_G1_proj_oneHalf , gpow);  // 1/2
    for(int j=0; j<halfN; j++) {
      bls12_381_G1_proj_add( src +  j* GRP_NLIMBS , src + (j+halfN)*GRP_NLIMBS , buf + j        *GRP_NLIMBS  );    // x + y
      bls12_381_G1_proj_sub( src +  j* GRP_NLIMBS , src + (j+halfN)*GRP_NLIMBS , buf + (j+halfN)*GRP_NLIMBS  );    // x - y
      bls12_381_G1_proj_scl_Fr_mont( bls12_381_G1_proj_oneHalf , buf + j        *GRP_NLIMBS , buf + j        *GRP_NLIMBS );    // (x + y) /  2
      bls12_381_G1_proj_scl_Fr_mont( gpow                    , buf + (j+halfN)*GRP_NLIMBS , buf + (j+halfN)*GRP_NLIMBS );    // (x - y) / (2*g^k)
      bls12_381_Fr_mont_mul_inplace( gpow , ginv );      
    }

    bls12_381_Fr_mont_sqr( gen, gpow );  // gen^2
    bls12_381_G1_proj_fft_inverse_noalloc( m-1 , tgt_stride<<1 , gpow , buf                    , buf + N*GRP_NLIMBS , tgt                         );
    bls12_381_G1_proj_fft_inverse_noalloc( m-1 , tgt_stride<<1 , gpow , buf + halfN*GRP_NLIMBS , buf + N*GRP_NLIMBS , tgt + tgt_stride*GRP_NLIMBS );
...
// `gen` should be the generator of the multiplicative subgroup (of the scalar field) sized `N`, in _Montgomery_ representation
// NOTE: we normalize the results
void bls12_381_G1_proj_fft_inverse(int m, const uint64_t *gen, const uint64_t *src, uint64_t *tgt) {
  int N = (1<<m);
  uint64_t *buf = malloc( 8*GRP_NLIMBS * (2*N) );
  assert( buf !=0 );
  bls12_381_G1_proj_fft_inverse_noalloc( m, 1, gen, src, buf, tgt );
  free(buf);
  for(int i=0; i<N; i++) { 
    bls12_381_G1_proj_normalize_inplace( tgt + i*GRP_NLIMBS );
  }
