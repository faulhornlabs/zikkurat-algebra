...
void bls12_381_G1_scale_by_B(const uint64_t *src, uint64_t *tgt ) {
  uint64_t tmp[6];
  bls12_381_p_mont_add( src, src, tmp );
  bls12_381_p_mont_add( tmp, tmp, tgt );
}

void bls12_381_G1_scale_by_B_inplace( uint64_t *tgt ) {
  uint64_t tmp[6];
...
void bls12_381_G1_scale_by_3B_inplace( uint64_t *tgt ) {
  bls12_381_G1_scale_by_3B( tgt , tgt );
}

void bls12_381_G1_normalize( const uint64_t *src1, uint64_t *tgt ) {
  uint64_t zinv[6];
...
void bls12_381_G1_from_aff( const uint64_t *src1 , uint64_t *tgt ) {
  memcpy( tgt, src1, 96 );
  bls12_381_p_mont_set_one( Z3 );
}
...
void bls12_381_G1_to_aff( const uint64_t *src1 , uint64_t *tgt ) {
  uint64_t zinv[6];
  bls12_381_p_mont_inv( Z1, zinv );
  bls12_381_p_mont_mul( X1, zinv, X3 );
  bls12_381_p_mont_mul( Y1, zinv, Y3 );
...
void bls12_381_G1_copy( const uint64_t *src1 , uint64_t *tgt ) {
  memcpy( tgt, src1, 144 );
}

uint8_t bls12_381_G1_is_infinity ( const uint64_t *src1 ) {
  return ( ( bls12_381_p_mont_is_zero( Z1 )) &&
           (!bls12_381_p_mont_is_zero( Y1 )) &&
           ( bls12_381_p_mont_is_zero( X1 )) );
}

void bls12_381_G1_set_infinity ( uint64_t *tgt ) {
...
uint8_t bls12_381_G1_is_on_curve ( const uint64_t *src1 ) {
  uint64_t  ZZ[6];
  uint64_t acc[6];
  uint64_t tmp[6];
  ...
  bls12_381_G1_scale_by_B_inplace( tmp );   // B*Z^3
...
uint8_t bls12_381_G1_is_in_subgroup ( const uint64_t *src1 ) {
  uint64_t tmp[18];
  if (!bls12_381_G1_is_on_curve(src1)) {
    return 0;
  }
  else {
    bls12_381_G1_scl_Fr( bls12_381_G1_cofactor , src1 , tmp );
    return bls12_381_G1_is_infinity( tmp );
  }
}

// negates an elliptic curve point
void bls12_381_G1_neg( const uint64_t *src, uint64_t *tgt ) {
  memcpy( tgt, src, 144 );
  bls12_381_p_mont_neg_inplace( Y3 );
}

// negates an elliptic curve point
void bls12_381_G1_neg_inplace( uint64_t *tgt ) {
  bls12_381_p_mont_neg_inplace( Y3 );
}

// doubles an elliptic curve point
// https://hyperelliptic.org/EFD/g1p/auto-shortw-projective.html#doubling-dbl-2007-bl
void bls12_381_G1_dbl( const uint64_t *src1, uint64_t *tgt ) {
  uint64_t  XX[6];
  uint64_t  ZZ[6];
  uint64_t   w[6];
  uint64_t   s[6];
...
// doubles an elliptic curve point
void bls12_381_G1_dbl_inplace( uint64_t *tgt ) {
  bls12_381_G1_dbl( tgt , tgt );
}

// adds two elliptic curve points
// https://hyperelliptic.org/EFD/g1p/auto-shortw-projective.html#addition-add-2015-rcb
void bls12_381_G1_add( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  uint64_t t0[6];
  uint64_t t1[6];
  uint64_t t2[6];
  uint64_t t3[6];
  ...
  bls12_381_G1_scale_by_A ( t4, Z3 );          // Z3 = a*t4 
  bls12_381_G1_scale_by_3B( t2, X3 );          // X3 = b3*t2
  bls12_381_p_mont_add_inplace( Z3, X3 );          // Z3 = X3+Z3
  ...
  bls12_381_G1_scale_by_A_inplace ( t2 );      // t2 = a*t2 
  bls12_381_G1_scale_by_3B_inplace( t4 );      // t4 = b3*t4
  bls12_381_p_mont_add_inplace( t1, t2 );          // t1 = t1+t2
  bls12_381_p_mont_sub_inplace_reverse( t2, t0 );  // t2 = t0-t2
  bls12_381_G1_scale_by_A_inplace ( t2 );      // t2 = a*t2 
  bls12_381_p_mont_add_inplace( t4, t2 );          // t4 = t4+t2
...
void bls12_381_G1_add_inplace( uint64_t *tgt, const uint64_t *src2 ) {
  bls12_381_G1_add( tgt, src2, tgt);
}

void bls12_381_G1_sub( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  uint64_t tmp[18];
  bls12_381_G1_neg( src2, tmp );
  bls12_381_G1_add( src1, tmp, tgt );
}

void bls12_381_G1_sub_inplace( uint64_t *tgt, const uint64_t *src2 ) {
  uint64_t tmp[18];
  bls12_381_G1_neg( src2, tmp );
  bls12_381_G1_add( tgt , tmp, tgt );
}
...
void bls12_381_G1_madd_proj_aff( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  uint64_t   u[6];
  uint64_t  uu[6];
  uint64_t   v[6];
  uint64_t  vv[6];
...
// https://hyperelliptic.org/EFD/g1p/auto-shortw-projective.html#addition-add-2015-rcb
void bls12_381_G1_madd_aff_proj( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  bls12_381_G1_madd_proj_aff( src2, src1, tgt );
}

// adds to a projective point (tgt) an affine point (src2), in place
void bls12_381_G1_madd_inplace( uint64_t *tgt, const uint64_t *src2 ) {
  bls12_381_G1_madd_proj_aff( tgt, src2, tgt );
}
...
// naive algorithm
void bls12_381_G1_scl_naive(const uint64_t *expo, const uint64_t *grp, uint64_t *tgt, int expo_len) {

  uint64_t dbl[3*NLIMBS_P];
  bls12_381_G1_copy( grp, dbl );              // dbl := grp
  bls12_381_G1_set_infinity( tgt );           // tgt := infinity
  ...
  for(int i=0; i<=s; i++) {
    ...
    for(int j=0; j<64; j++) {
      if (e & 1) { 
        bls12_381_G1_add( tgt, dbl, tgt ); 
      }
      bls12_381_G1_dbl( dbl, dbl );
      e = e >> 1;
    }
  }
}
...
void bls12_381_G1_precalc_expos_window_16( const uint64_t *grp, uint64_t *table ) {
  bls12_381_G1_copy( grp              , TBL( 1) );           //  1*g
  bls12_381_G1_dbl ( TBL(1)           , TBL( 2) );           //  2*g
  bls12_381_G1_dbl ( TBL(2)           , TBL( 4) );           //  4*g
  bls12_381_G1_dbl ( TBL(4)           , TBL( 8) );           //  8*g
  bls12_381_G1_add ( TBL(1) , TBL( 2) , TBL( 3) );           //  3*g
  bls12_381_G1_dbl ( TBL(3) ,           TBL( 6) );           //  6*g
  bls12_381_G1_dbl ( TBL(6) ,           TBL(12) );           // 12*g
  bls12_381_G1_add ( TBL(1) , TBL( 4) , TBL( 5) );           //  5*g
  bls12_381_G1_dbl ( TBL(5) ,           TBL(10) );           // 10*g
  bls12_381_G1_add ( TBL(1) , TBL( 6) , TBL( 7) );           //  7*g
  bls12_381_G1_dbl ( TBL(7) ,           TBL(14) );           // 14*g
  bls12_381_G1_add ( TBL(1) , TBL( 8) , TBL( 9) );           //  9*g
  bls12_381_G1_add ( TBL(1) , TBL(10) , TBL(11) );           // 11*g
  bls12_381_G1_add ( TBL(1) , TBL(12) , TBL(13) );           // 13*g
  bls12_381_G1_add ( TBL(1) , TBL(14) , TBL(15) );           // 15*g
}

// computes `expo*grp` (or `grp^expo` in multiplicative notation)
// where `grp` is a group element in G1, and `expo` is a (non-negative) bigint
// generic windowed algo, 4-bit windows
void bls12_381_G1_scl_windowed(const uint64_t *expo, const uint64_t *grp, uint64_t *tgt, int expo_len) {

  // precalculate [ k*g | k <- [1..15] ]
  uint64_t table[15*3*NLIMBS_P];
  bls12_381_G1_precalc_expos_window_16( grp, table );
  ...
  for(int i=s; i>=0; i--) {
    ...
    for(int j=0; j<16; j++) {
      ...
      int k = (e >> 60);
      if (k) { 
        bls12_381_G1_add_inplace( tgt, TBL(k) ); 
      }
      e = e << 4;
    }
  }
