...
// elliptic curve "BN128 ( Fp ) " in projective coordinates, Montgomery field representation
//
...
const uint64_t bn128_G1_proj_const_A[4] = { 0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000 };
const uint64_t bn128_G1_proj_const_B[4] = { 0x7a17caa950ad28d7, 0x1f6ac17ae15521b9, 0x334bea4e696bd284, 0x2a1f6744ce179d8e };
const uint64_t bn128_G1_proj_const_3B[4] = { 0xf60647ce410d7ff7, 0x2f3d6f4dd31bd011, 0x2943337e3940c6d1, 0x1d9598e8a7e39857 };
...
void bn128_G1_proj_batch_from_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bn128_G1_proj_from_affine(p,q);
    p += 2*NLIMBS_P;
    q += 3*NLIMBS_P;
  }
}
...
void bn128_G1_proj_batch_to_affine( int N, const uint64_t *src , uint64_t *tgt ) {
  const uint64_t *p = src;
  uint64_t *q = tgt;
  for(int i=0; i<N; i++) {
    bn128_G1_proj_to_affine(p,q);
    p += 3*NLIMBS_P;
    q += 2*NLIMBS_P;
  }
}
...
//  - projective (proj) Montgomery point
//
void bn128_G1_proj_MSM_binary_coeff_proj_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  bn128_G1_proj_set_infinity( tgt );
  for(int i=0; i<npoints; i++) { 
    if (expos[i]) {
      // add to the running sum
      bn128_G1_proj_madd_inplace( tgt , grps  + i*2*NLIMBS_P );
    }
  }
}

void bn128_G1_proj_MSM_binary_coeff_affine_out(int npoints, const uint8_t *expos, const uint64_t *grps, uint64_t *tgt) {
  uint64_t tmp[3*NLIMBS_P];
  bn128_G1_proj_MSM_binary_coeff_proj_out(npoints, expos, grps, tmp);
  bn128_G1_proj_to_affine(tmp, tgt);
}
...
//
void bn128_G1_proj_multi_MSM_binary_coeff_proj_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  ...
  for(int i=0; i<nrows; i++) { 
    bn128_G1_proj_set_infinity( tgt + i*3*NLIMBS_P );
  }
  
  uint64_t *precalc = (uint64_t*) malloc( (1<<c) * 8*3*NLIMBS_P );
  assert( precalc != 0 );
  bn128_G1_proj_set_infinity( precalc );
  ...
  for(int k=0; k<nwindows; k++) {
    ...
    for(int j=0; j<C; j++) {
      if (j==0) {
        bn128_G1_proj_from_affine( gptr , PRECALC(1) );
      }
      else {
        int m = (1<<j);
        for(int i=0; i<m; i++) {
          bn128_G1_proj_madd_proj_aff( PRECALC(i) , gptr , PRECALC(i+m) );
        }
      }
      gptr += 2*NLIMBS_P;
    }
    ...
    for(int i=0; i<nrows; i++) {
      ...
      if (idx > 0) {
        // add to the running sum
        bn128_G1_proj_add_inplace( tgt + i*3*NLIMBS_P , PRECALC(idx) );
      }
    }
...
void bn128_G1_proj_multi_MSM_binary_coeff_affine_out(int nrows, int npoints, const uint8_t **coeffs, const uint64_t *grps, uint64_t *tgt) {
  uint64_t *tmp = (uint64_t*) malloc( nrows * 8*3*NLIMBS_P );
  assert( tmp != 0 );
  bn128_G1_proj_multi_MSM_binary_coeff_proj_out( nrows, npoints, coeffs, grps, tmp);
  for(int i=0; i<nrows; i++) {
    bn128_G1_proj_to_affine( tmp + i*3*NLIMBS_P, tgt + i*2*NLIMBS_P );
  }
  free(tmp);
}
