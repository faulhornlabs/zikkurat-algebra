...
void bls12_381_arr_mont_concat( int K , int *ns, const uint64_t **srcs, uint64_t *tgt ) {
  uint64_t *q = tgt;
  for(int k=0; k<K; k++) {
...
void bls12_381_arr_mont_mul_by_powers ( int n, const uint64_t *coeffA, const uint64_t *coeffB, const uint64_t *src, uint64_t *tgt ) {
  if (n==0) return;
  uint64_t x[ELEM_NWORDS];
  bls12_381_Fr_mont_copy( coeffA , x );
  for(int i=1; i<n; i++) {
  bls12_381_Fr_mont_mul( SRC(i), x, TGT(i) );
  bls12_381_Fr_mont_mul_inplace( x, coeffB );
  }
}
