...
void bn128_Fr_mont_batch_from_std( int N, const uint64_t *src, uint64_t *tgt) {
  const uint64_t *p = src;
  uint64_t       *q = tgt;
  for(int i=0; i<N; i++) {
    bn128_Fr_mont_from_std(p,q);
    p += NLIMBS;
    q += NLIMBS;
  }
}
...
void bn128_Fr_mont_batch_to_std( int N, const uint64_t *src, uint64_t *tgt) {
  const uint64_t *p = src;
  uint64_t       *q = tgt;
  for(int i=0; i<N; i++) {
