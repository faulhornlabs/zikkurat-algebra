...
// divides by 2
void bn128_Fp_mont_div_by_2( const uint64_t *src, uint64_t *tgt ) {
  return bn128_Fp_std_div_by_2(src,tgt);
}

// divides by 2, inplace
void bn128_Fp_mont_div_by_2_inplace( uint64_t *tgt ) {
  return bn128_Fp_std_div_by_2_inplace(tgt);
}
...
void bn128_Fp_mont_batch_from_std( int N, const uint64_t *src, uint64_t *tgt) {
  const uint64_t *p = src;
  uint64_t       *q = tgt;
  for(int i=0; i<N; i++) {
    bn128_Fp_mont_from_std(p,q);
    p += NLIMBS;
    q += NLIMBS;
  }
}
...
void bn128_Fp_mont_batch_to_std( int N, const uint64_t *src, uint64_t *tgt) {
  const uint64_t *p = src;
  uint64_t       *q = tgt;
  for(int i=0; i<N; i++) {
    bn128_Fp_mont_to_std(p,q);
    p += NLIMBS;
    q += NLIMBS;
  }
};
