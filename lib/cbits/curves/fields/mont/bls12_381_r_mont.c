...
void bls12_381_r_mont_pow_uint64( const uint64_t *src, uint64_t exponent, uint64_t *tgt ) {
  uint64_t e = exponent;
  uint64_t sqr[4];
  bls12_381_r_mont_copy( src, sqr );                 // sqr := src
  bls12_381_r_mont_set_one( tgt );                   // tgt := 1
  while(e!=0) {
...
void bls12_381_r_mont_batch_inv( int n, const uint64_t *src, uint64_t *tgt ) {
  assert( n >= 1 );
  ...
  bls12_381_r_mont_copy( SRC(0) , PROD(0) );
  for(int i=1; i<n; i++) {
    bls12_381_r_mont_mul( PROD(i-1) , SRC(i) , PROD(i) );
  }
  
  // compute inverses of partial products 1/(a[0]*a[1]*...*a[k]) for all k
  bls12_381_r_mont_inv( PROD(n-1) , RECIP(n-1) );
  for(int i=n-2; i>=0; i--) {
    bls12_381_r_mont_mul( RECIP(i+1) , SRC(i+1) , RECIP(i) );
  }
  
  // compute the inverses 1/a[k] for all k
  bls12_381_r_mont_copy( RECIP(0) , TGT(0) );
  for(int i=1; i<n; i++) {
    bls12_381_r_mont_mul( RECIP(i) , PROD(i-1) , TGT(i) );
  }
