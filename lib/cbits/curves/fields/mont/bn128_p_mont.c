...
void bn128_p_mont_pow_uint64( const uint64_t *src, uint64_t exponent, uint64_t *tgt ) {
  uint64_t e = exponent;
  uint64_t sqr[4];
  bn128_p_mont_copy( src, sqr );                 // sqr := src
  bn128_p_mont_set_one( tgt );                   // tgt := 1
  while(e!=0) {
...
void bn128_p_mont_batch_inv( int n, const uint64_t *src, uint64_t *tgt ) {
  assert( n >= 1 );
  ...
  bn128_p_mont_copy( SRC(0) , PROD(0) );
  for(int i=1; i<n; i++) {
    bn128_p_mont_mul( PROD(i-1) , SRC(i) , PROD(i) );
  }
  
  // compute inverses of partial products 1/(a[0]*a[1]*...*a[k]) for all k
  bn128_p_mont_inv( PROD(n-1) , RECIP(n-1) );
  for(int i=n-2; i>=0; i--) {
    bn128_p_mont_mul( RECIP(i+1) , SRC(i+1) , RECIP(i) );
  }
  
  // compute the inverses 1/a[k] for all k
  bn128_p_mont_copy( RECIP(0) , TGT(0) );
  for(int i=1; i<n; i++) {
    bn128_p_mont_mul( RECIP(i) , PROD(i-1) , TGT(i) );
  }
...
uint8_t bn128_p_mont_is_zero( const uint64_t *src ) {
  return bigint256_is_zero( src );
}

uint8_t bn128_p_mont_is_one( const uint64_t *src ) {
  return bigint256_is_equal( src, bn128_p_mont_R_modp );
}

uint8_t bn128_p_mont_is_equal( const uint64_t *src1, const uint64_t *src2 ) {
  return bigint256_is_equal( src1 , src2 );
}

void bn128_p_mont_set_zero( uint64_t *tgt ) {
  bigint256_set_zero( tgt );
}

void bn128_p_mont_set_one( uint64_t *tgt) {
  bigint256_copy( bn128_p_mont_R_modp , tgt );
}

void bn128_p_mont_copy( const uint64_t *src, uint64_t *tgt ) {
