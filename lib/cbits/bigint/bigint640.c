...
// unsigned big integers composed of 10 64-bit words
//
...
uint8_t bigint640_is_zero(const uint64_t *src) {
  return ( (src[0] == 0) && (src[1] == 0) && (src[2] == 0) && (src[3] == 0) && (src[4] == 0) && (src[5] == 0) && (src[6] == 0) && (src[7] == 0) && (src[8] == 0) && (src[9] == 0)) ;
}

uint8_t bigint640_is_one(const uint64_t *src) {
  return ( (src[0] == 1) && (src[1] == 0) && (src[2] == 0) && (src[3] == 0) && (src[4] == 0) && (src[5] == 0) && (src[6] == 0) && (src[7] == 0) && (src[8] == 0) && (src[9] == 0)) ;
}

uint8_t bigint640_is_equal(const uint64_t *src1, const uint64_t *src2 ) {
  return ( (src1[0] == src2[0]) && (src1[1] == src2[1]) && (src1[2] == src2[2]) && (src1[3] == src2[3]) && (src1[4] == src2[4]) && (src1[5] == src2[5]) && (src1[6] == src2[6]) && (src1[7] == src2[7]) && (src1[8] == src2[8]) && (src1[9] == src2[9])) ;
}

void bigint640_set_zero(uint64_t *tgt) {
  memset( tgt, 0, 80 );
}

void bigint640_set_one(uint64_t *tgt) {
  memset( tgt, 0, 80 );
  tgt[0] = 1;
}

void bigint640_set_small(uint64_t *tgt, uint64_t s) {
  memset( tgt, 0, 80 );
  tgt[0] = s;
}

void bigint640_copy(const uint64_t *src, uint64_t *tgt) {
  if (src != tgt) { memcpy( tgt, src, 80 ); }
}

void bigint640_print(const uint64_t *what, int underscore_separators) {
  if (underscore_separators) {
    for(int i=0; i<10; i++) {
      printf("%016llx_", what[9-i]);
    }
  }
  else {
    for(int i=0; i<10; i++) {
      printf("%016llx", what[9-i]);
    }
  }
}

void bigint640_debug_print(const char *txt, const uint64_t *what) {
  printf("%s = 0x",txt);
  bigint640_print( what, 1 );
  printf("\n");
}

// increments bigint by 1, inplace
uint8_t bigint640_inc_inplace( uint64_t *tgt ) {
  uint8_t c = 0;
...
uint8_t bigint640_dec_inplace( uint64_t *tgt ) {
  uint8_t b = 0;
...
// negates a bigint
void bigint640_neg( const uint64_t *src, uint64_t *tgt ) {
  uint8_t b = 0;
...
void bigint640_neg_inplace( uint64_t *tgt ) {
  uint8_t b = 0;
...
// adds two (unsigned) big integers made up from 10 limbs (64-bit words)
uint8_t bigint640_add( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  uint8_t c = 0;
...
// adds two big integers made up from 10 limbs (64-bit words)
uint8_t bigint640_add_inplace( uint64_t *tgt, const uint64_t *src2 ) {
  uint8_t c = 0;
...
// subtracts two (unsigned) big integers made up from 10 limbs (64-bit words)
uint8_t bigint640_sub( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  uint8_t b = 0;
...
// subtracts two big integers made up from 10 limbs (64-bit words)
uint8_t bigint640_sub_inplace( uint64_t *tgt, const uint64_t *src2 ) {
  uint8_t b = 0;
...
// tgt := src - tgt
uint8_t bigint640_sub_inplace_reverse( uint64_t *tgt, const uint64_t *src1 ) {
  uint8_t b = 0;
...
// multiplies an (unsigned) big integers of 10 limbs with a 64-bit word
// note: `tgt` must have space for 11 limbs!
void bigint640_scale( uint64_t z, const uint64_t *src, uint64_t *tgt) {
  uint8_t c;
  __uint128_t x;
  uint64_t hi,lo;
  tgt[0] = 0;
...
// squares an (unsigned) big integer of 10 limbs
// note: `tgt` must have space for 20 limbs!
void bigint640_sqr( const uint64_t *src, uint64_t *tgt ) {
  __uint128_t prod;
  uint64_t prod_hi, prod_lo;
  uint64_t carry;
  for(int m=0; m<20; m++) { tgt[m] = 0; }
  // *** m = 0 ***
  carry = 0;
...
// multiplies two (unsigned) big integers of 10 limbs
// note: `tgt` must have space for 20 limbs!
void bigint640_mul( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  __uint128_t prod;
  uint64_t prod_hi, prod_lo;
  uint64_t carry;
  for(int m=0; m<20; m++) { tgt[m] = 0; }
...
// multiplies two (unsigned) big integers of 10 limbs,
// and *truncates* the result 10 limbs
// (so this gives the ring of integers modulo 2^640)
void bigint640_mul_truncated( const uint64_t *src1, const uint64_t *src2, uint64_t *tgt ) {
  __uint128_t prod;
  uint64_t prod_hi, prod_lo;
  uint64_t carry;
...
// squares an (unsigned) big integers of 10 limbs,
// and *truncates* the result 10 limbs
// (so this gives the ring of integers modulo 2^640)
void bigint640_sqr_truncated( const uint64_t *src1, uint64_t *tgt ) {
  __uint128_t prod;
  uint64_t prod_hi, prod_lo;
  uint64_t carry;
  for(int m=0; m<10; m++) { tgt[m] = 0; }
...
// shift left by 1 bit
uint8_t bigint640_shift_left_by_1( const uint64_t *src, uint64_t *tgt) {
  uint64_t tmp[10];
  tmp[0] = (src[0] << 1) ;
...
uint64_t bigint640_shift_left_by_small( const uint64_t *src, uint64_t *tgt, int by ) {
  assert( (by >= 0) && (by < 64) );
  uint64_t tmp[10];
  tmp[0] = (src[0] << by) ;
  ...
  uint64_t c = src[9] >> (64-by);
  memcpy( tgt, tmp, 80 );
  return c;
}

// shift left by k bits, for k >= 0
void bigint640_shift_left_by_k( const uint64_t *src, uint64_t *tgt, int by ) {
  assert(by >= 0);
  int small = by % 64;
  int move  = by / 64;
  if (by >= 640) {
    bigint640_set_zero(tgt);
  } 
  else {
    for(int i=0    ; i<move; i++) { tgt[i] = 0; }
    for(int i=move ; i<10     ; i++) { tgt[i] = src[i-move]; }
    bigint640_shift_left_by_small(tgt, tgt, small);
  }
}

// shift right by 1 bit
uint8_t bigint640_shift_right_by_1( const uint64_t *src, uint64_t *tgt) {
  uint64_t tmp[10];
  tmp[9] = (src[9] >> 1) ;
...
uint64_t bigint640_shift_right_by_small( const uint64_t *src, uint64_t *tgt, int by ) {
  assert( (by >= 0) && (by < 64) );
  uint64_t tmp[10];
  tmp[9] = (src[9] >> by) ;
...
void bigint640_shift_right_by_k( const uint64_t *src, uint64_t *tgt, int by ) {
  assert(by >= 0);
  int small = by % 64;
  int move  = by / 64;
  if (by >= 640) {
    ...
    for(int i=0     ; i<10-move; i++) { tgt[i] = src[i+move]; }
    for(int i=10-move; i<10     ; i++) { tgt[i] = 0;           }
    bigint640_shift_right_by_small(tgt, tgt, small);
  }
}
