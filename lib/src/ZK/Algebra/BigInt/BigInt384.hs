...
instance L.Flat BigInt384 where
  sizeInBytes  _pxy = 48
  sizeInQWords _pxy = 6
  withFlat (MkBigInt384 fptr) = withForeignPtr fptr
  makeFlat = L.makeFlatGeneric MkBigInt384 6
...
instance M.Rnd BigInt384 where
  rndIO = rnd

instance C.Ring BigInt384 where
  ringName _ = "BigInt384"
  ringSize _ = 39402006196394479212279040100143613805079739270465446667948293404245721771497210611414266254884915640806627990306816
  isZero = ZK.Algebra.BigInt.BigInt384.isZero
  isOne  = ZK.Algebra.BigInt.BigInt384.isOne
  zero   = ZK.Algebra.BigInt.BigInt384.zero
  one    = ZK.Algebra.BigInt.BigInt384.one
  square = ZK.Algebra.BigInt.BigInt384.sqr
