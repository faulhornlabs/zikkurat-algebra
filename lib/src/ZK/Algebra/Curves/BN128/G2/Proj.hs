...
instance C.Curve G2 where
  ...
  isInfinity  = ZK.Algebra.Curves.BN128.G2.Proj.isInfinity
...
instance C.ProjCurve G2 where
  ...
  binaryMSM       = ZK.Algebra.Curves.BN128.G2.Proj.msmBinary
  binaryMultiMSM  = ZK.Algebra.Curves.BN128.G2.Proj.binaryMultiMSM
...
foreign import ccall unsafe "bn128_G2_proj_MSM_binary_coeff_proj_out" c_bn128_G2_proj_MSM_binary_coeff_proj_out :: CInt -> Ptr Word8  -> Ptr Word64 -> Ptr Word64 -> IO ()

foreign import ccall unsafe "bn128_G2_proj_multi_MSM_binary_coeff_proj_out" c_bn128_G2_proj_multi_MSM_binary_coeff_proj_out :: CInt -> CInt -> Ptr (Ptr Word8) -> Ptr Word64 -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bn128_G2_proj_multi_MSM_binary_coeff_affine_out" c_bn128_G2_proj_multi_MSM_binary_coeff_affine_out :: CInt -> CInt -> Ptr (Ptr Word8) -> Ptr Word64 -> Ptr Word64 -> IO ()
...
-- 
msmBinary :: FlatArray L.Bit -> FlatArray ZK.Algebra.Curves.BN128.G2.Affine.G2 -> G2
msmBinary (MkFlatArray n1 fptr1) (MkFlatArray n2 fptr2)
  ...
  | otherwise  = unsafePerformIO $ do
      ...
      withForeignPtr fptr1 $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bn128_G2_proj_MSM_binary_coeff_proj_out (fromIntegral n1) (castPtr ptr1) ptr2 ptr3
      return (MkG2 fptr3)
...
binaryMultiMSM :: [FlatArray L.Bit] -> FlatArray ZK.Algebra.Curves.BN128.G2.Affine.G2 -> FlatArray G2
  ...
  | otherwise  = unsafePerformIO $ do
      ...
      L.withFlatArrays flatarrs $ \list -> do
        ...
        withArray ptrs $ \ptr_ptrs -> do
          withForeignPtr fptr2 $ \ptr2 -> do
            withForeignPtr fptr3 $ \ptr3 -> do
              c_bn128_G2_proj_multi_MSM_binary_coeff_proj_out (fromIntegral narrs) (fromIntegral n1) ptr_ptrs ptr2 ptr3
...
-- 
binaryMultiMSM_ :: [FlatArray L.Bit] -> FlatArray ZK.Algebra.Curves.BN128.G2.Affine.G2 -> FlatArray ZK.Algebra.Curves.BN128.G2.Affine.G2
binaryMultiMSM_ flatarrs (MkFlatArray n2 fptr2)
  | n1 /= n2   = error "binaryMultiMSM: incompatible array dimensions"
  | otherwise  = unsafePerformIO $ do
      fptr3 <- mallocForeignPtrArray (narrs * 16)
      L.withFlatArrays flatarrs $ \list -> do
        let ptrs = map (castPtr . snd) list :: [Ptr Word8]
        withArray ptrs $ \ptr_ptrs -> do
          withForeignPtr fptr2 $ \ptr2 -> do
            withForeignPtr fptr3 $ \ptr3 -> do
              c_bn128_G2_proj_multi_MSM_binary_coeff_affine_out (fromIntegral narrs) (fromIntegral n1) ptr_ptrs ptr2 ptr3
      return (MkFlatArray narrs fptr3)
  where
    narrs = length flatarrs
    ns    = map L.flatArrayLength flatarrs
