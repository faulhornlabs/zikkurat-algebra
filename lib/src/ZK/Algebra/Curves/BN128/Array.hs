...
instance V.Vector (FlatArray Fr) where
  ...
  vecConcat   = ZK.Algebra.Curves.BN128.Array.concat
...
foreign import ccall unsafe "bn128_arr_mont_concat" c_bn128_arr_mont_concat :: CInt -> Ptr CInt -> Ptr (Ptr Word64) -> Ptr Word64 -> IO ()
    ...
    withArray (map (fromIntegral :: Int -> CInt) ns) $ \ns_ptr -> do 
      withArray ptrs $ \ptrs_ptr -> do
        withForeignPtr fptr3 $ \tgt_ptr -> do
          c_bn128_arr_mont_concat (fromIntegral $ length list) ns_ptr ptrs_ptr tgt_ptr
