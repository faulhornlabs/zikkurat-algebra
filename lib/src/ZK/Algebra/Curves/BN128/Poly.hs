...
instance P.Univariate Poly where
  ...
  mkPolyFlat      = ZK.Algebra.Curves.BN128.Poly.mkPolyFlatArr
  coeffs          = ZK.Algebra.Curves.BN128.Poly.coeffs
  coeffsArr       = ZK.Algebra.Curves.BN128.Poly.coeffsArr
  coeffsFlatArr   = ZK.Algebra.Curves.BN128.Poly.coeffsFlatArr
  polyLongDiv     = ZK.Algebra.Curves.BN128.Poly.longDiv
  polyQuot        = ZK.Algebra.Curves.BN128.Poly.quot
  polyRem         = ZK.Algebra.Curves.BN128.Poly.rem
  divByVanishing  = ZK.Algebra.Curves.BN128.Poly.divByVanishing
  quotByVanishing = ZK.Algebra.Curves.BN128.Poly.quotByVanishing
...
foreign import ccall unsafe "bn128_poly_mont_long_div" c_bn128_poly_mont_long_div :: CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bn128_poly_mont_quot"     c_bn128_poly_mont_quot     :: CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bn128_poly_mont_rem"      c_bn128_poly_mont_rem      :: CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> IO ()
    ...
    withForeignPtr fptr2 $ \ptr2 -> do
      withForeignPtr fptr3 $ \ptr3 -> do
        withForeignPtr fptr4 $ \ptr4 -> do
          c_bn128_poly_mont_long_div (fromIntegral n1) ptr1 (fromIntegral n2) ptr2 (fromIntegral nq) ptr3 (fromIntegral nr) ptr4
...
quot poly1@(XPoly n1 fptr1) poly2@(XPoly n2 fptr2) = unsafePerformIO $ do
  ...
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      withForeignPtr fptr3 $ \ptr3 -> do
        c_bn128_poly_mont_quot (fromIntegral n1) ptr1 (fromIntegral n2) ptr2 (fromIntegral nq) ptr3
...
rem poly1@(XPoly n1 fptr1) poly2@(XPoly n2 fptr2) = unsafePerformIO $ do
  ...
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      withForeignPtr fptr4 $ \ptr4 -> do
        c_bn128_poly_mont_rem (fromIntegral n1) ptr1 (fromIntegral n2) ptr2 (fromIntegral nr) ptr4
  return (XPoly nr fptr4)

foreign import ccall unsafe "bn128_poly_mont_div_by_vanishing"  c_bn128_poly_mont_div_by_vanishing  :: CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bn128_poly_mont_quot_by_vanishing" c_bn128_poly_mont_quot_by_vanishing :: CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> CInt -> Ptr Word64 -> IO Word8
...
divByVanishing poly1@(XPoly n1 fptr1) (expo_n, MkFr fptr2) = unsafePerformIO $ do
  ...
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      withForeignPtr fptr3 $ \ptr3 -> do
        withForeignPtr fptr4 $ \ptr4 -> do
          c_bn128_poly_mont_div_by_vanishing (fromIntegral n1) ptr1 (fromIntegral expo_n) ptr2 (fromIntegral nq) ptr3 (fromIntegral nr) ptr4
  return (XPoly nq fptr3, XPoly nr fptr4)
...
quotByVanishing poly1@(XPoly n1 fptr1) (expo_n, MkFr fptr2) = unsafePerformIO $ do
  ...
  cret <- withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      withForeignPtr fptr3 $ \ptr3 -> do
        c_bn128_poly_mont_quot_by_vanishing (fromIntegral n1) ptr1 (fromIntegral expo_n) ptr2 (fromIntegral nq) ptr3
  return $ if (cret /= 0)
...
forwardNTT sg (MkPoly (MkFlatArray n fptr2))
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bn128_poly_mont_ntt_forward (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
...
inverseNTT sg (MkFlatArray n fptr2)
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bn128_poly_mont_ntt_inverse (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
        ...
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            withForeignPtr fptr_eta $ \ptr_eta -> do
              c_bn128_poly_mont_ntt_forward_shifted ptr_eta (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
      return (MkFlatArray n fptr3)
