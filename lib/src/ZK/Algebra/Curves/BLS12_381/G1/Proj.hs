...
forwardFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise              = unsafePerformIO $ do
      ...
      L.withFlat (subgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_proj_fft_forward (fromIntegral $ M.fromLog2 $ subgroupLogSize sg) ptr1 ptr2 ptr3
...
inverseFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise              = unsafePerformIO $ do
      ...
      L.withFlat (subgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_proj_fft_inverse (fromIntegral $ M.fromLog2 $ subgroupLogSize sg) ptr1 ptr2 ptr3
      return (MkFlatArray n fptr3)
