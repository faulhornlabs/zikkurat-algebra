...
instance C.Curve G1 where
  ...
  isInfinity  = ZK.Algebra.Curves.BLS12_381.G1.Proj.isInfinity
  infinity    = ZK.Algebra.Curves.BLS12_381.G1.Proj.infinity
  curveSubgroupGen = ZK.Algebra.Curves.BLS12_381.G1.Proj.genG1
  scalarMul   = ZK.Algebra.Curves.BLS12_381.G1.Proj.sclFr
  msm         = ZK.Algebra.Curves.BLS12_381.G1.Proj.msmProj
  curveFFT    = ZK.Algebra.Curves.BLS12_381.G1.Proj.forwardFFT
  curveIFFT   = ZK.Algebra.Curves.BLS12_381.G1.Proj.inverseFFT
...
instance C.ProjCurve G1 where
  ...
  batchFromAffine = ZK.Algebra.Curves.BLS12_381.G1.Proj.batchFromAffine
  batchToAffine   = ZK.Algebra.Curves.BLS12_381.G1.Proj.batchToAffine
...
forwardFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      L.withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_proj_fft_forward (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
...
inverseFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      L.withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_proj_fft_inverse (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
      return (MkFlatArray n fptr3)
...
foreign import ccall unsafe "bls12_381_G1_proj_batch_from_affine" c_bls12_381_G1_proj_batch_from_affine :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()

{-# NOINLINE batchFromAffine #-}
batchFromAffine :: FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Affine.G1) -> FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Proj.G1)
batchFromAffine (MkFlatArray n fptr1) = unsafePerformIO $ do
  fptr2 <- mallocForeignPtrArray (n*18)
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      c_bls12_381_G1_proj_batch_from_affine (fromIntegral n) ptr1 ptr2
  return (MkFlatArray n fptr2)

foreign import ccall unsafe "bls12_381_G1_proj_batch_to_affine" c_bls12_381_G1_proj_batch_to_affine :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()

{-# NOINLINE batchToAffine #-}
batchToAffine :: FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Proj.G1) -> FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Affine.G1)
batchToAffine (MkFlatArray n fptr1) = unsafePerformIO $ do
  fptr2 <- mallocForeignPtrArray (n*12)
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      c_bls12_381_G1_proj_batch_to_affine (fromIntegral n) ptr1 ptr2
  return (MkFlatArray n fptr2)
