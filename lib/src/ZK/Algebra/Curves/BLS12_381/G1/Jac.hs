...
instance C.Curve G1 where
  ...
  msm         = ZK.Algebra.Curves.BLS12_381.G1.Jac.msmJac
  curveFFT    = ZK.Algebra.Curves.BLS12_381.G1.Jac.forwardFFT
  curveIFFT   = ZK.Algebra.Curves.BLS12_381.G1.Jac.inverseFFT
...
instance C.ProjCurve G1 where
  ...
  batchFromAffine = ZK.Algebra.Curves.BLS12_381.G1.Jac.batchFromAffine
  batchToAffine   = ZK.Algebra.Curves.BLS12_381.G1.Jac.batchToAffine
  ...
  affMSM     = ZK.Algebra.Curves.BLS12_381.G1.Jac.msm
  binaryMSM       = ZK.Algebra.Curves.BLS12_381.G1.Jac.msmBinary
  binaryMultiMSM  = ZK.Algebra.Curves.BLS12_381.G1.Jac.binaryMultiMSM
...
foreign import ccall unsafe "bls12_381_G1_jac_MSM_binary_coeff_jac_out" c_bls12_381_G1_jac_MSM_binary_coeff_jac_out :: CInt -> Ptr Word8  -> Ptr Word64 -> Ptr Word64 -> IO ()

foreign import ccall unsafe "bls12_381_G1_jac_multi_MSM_binary_coeff_jac_out" c_bls12_381_G1_jac_multi_MSM_binary_coeff_jac_out :: CInt -> CInt -> Ptr (Ptr Word8) -> Ptr Word64 -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bls12_381_G1_jac_multi_MSM_binary_coeff_affine_out" c_bls12_381_G1_jac_multi_MSM_binary_coeff_affine_out :: CInt -> CInt -> Ptr (Ptr Word8) -> Ptr Word64 -> Ptr Word64 -> IO ()
...
msmBinary (MkFlatArray n1 fptr1) (MkFlatArray n2 fptr2)
  ...
  | otherwise  = unsafePerformIO $ do
      ...
      withForeignPtr fptr1 $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_jac_MSM_binary_coeff_jac_out (fromIntegral n1) (castPtr ptr1) ptr2 ptr3
      return (MkG1 fptr3)
  ...
  | otherwise  = unsafePerformIO $ do
      ...
      L.withFlatArrays flatarrs $ \list -> do
        ...
        withArray ptrs $ \ptr_ptrs -> do
          withForeignPtr fptr2 $ \ptr2 -> do
            withForeignPtr fptr3 $ \ptr3 -> do
              c_bls12_381_G1_jac_multi_MSM_binary_coeff_jac_out (fromIntegral narrs) (fromIntegral n1) ptr_ptrs ptr2 ptr3
  ...
  | otherwise  = unsafePerformIO $ do
      ...
      L.withFlatArrays flatarrs $ \list -> do
        ...
        withArray ptrs $ \ptr_ptrs -> do
          withForeignPtr fptr2 $ \ptr2 -> do
            withForeignPtr fptr3 $ \ptr3 -> do
              c_bls12_381_G1_jac_multi_MSM_binary_coeff_affine_out (fromIntegral narrs) (fromIntegral n1) ptr_ptrs ptr2 ptr3
      return (MkFlatArray narrs fptr3)
  where
    narrs = length flatarrs
    ns    = map L.flatArrayLength flatarrs
...
foreign import ccall unsafe "bls12_381_G1_jac_fft_inverse" c_bls12_381_G1_jac_fft_inverse :: CInt -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> IO ()
foreign import ccall unsafe "bls12_381_G1_jac_fft_forward" c_bls12_381_G1_jac_fft_forward :: CInt -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> IO ()
...
forwardFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      L.withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_jac_fft_forward (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
...
inverseFFT sg (MkFlatArray n fptr2)
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      L.withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_G1_jac_fft_inverse (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
      return (MkFlatArray n fptr3)
...
foreign import ccall unsafe "bls12_381_G1_jac_batch_from_affine" c_bls12_381_G1_jac_batch_from_affine :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()

{-# NOINLINE batchFromAffine #-}
batchFromAffine :: FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Affine.G1) -> FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Jac.G1)
batchFromAffine (MkFlatArray n fptr1) = unsafePerformIO $ do
  fptr2 <- mallocForeignPtrArray (n*18)
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      c_bls12_381_G1_jac_batch_from_affine (fromIntegral n) ptr1 ptr2
  return (MkFlatArray n fptr2)

foreign import ccall unsafe "bls12_381_G1_jac_batch_to_affine" c_bls12_381_G1_jac_batch_to_affine :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()

{-# NOINLINE batchToAffine #-}
batchToAffine :: FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Jac.G1) -> FlatArray (ZK.Algebra.Curves.BLS12_381.G1.Affine.G1)
batchToAffine (MkFlatArray n fptr1) = unsafePerformIO $ do
  fptr2 <- mallocForeignPtrArray (n*12)
  withForeignPtr fptr1 $ \ptr1 -> do
    withForeignPtr fptr2 $ \ptr2 -> do
      c_bls12_381_G1_jac_batch_to_affine (fromIntegral n) ptr1 ptr2
  return (MkFlatArray n fptr2)
