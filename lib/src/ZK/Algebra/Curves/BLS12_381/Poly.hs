...
instance F.Ring Poly where
  ...
  square    = ZK.Algebra.Curves.BLS12_381.Poly.sqr
...
foreign import ccall unsafe "bls12_381_poly_mont_ntt_forward_asymmetric" c_bls12_381_poly_mont_ntt_forward_asymmetric :: CInt -> CInt -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> IO ()
...
forwardNTT sg (MkPoly (MkFlatArray n fptr2))
  ...
  | otherwise                 = unsafePerformIO $ do
      ...
      withFlat (fftSubgroupGen sg) $ \ptr1 -> do
        withForeignPtr fptr2 $ \ptr2 -> do
          withForeignPtr fptr3 $ \ptr3 -> do
            c_bls12_381_poly_mont_ntt_forward (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3
  ...
  | otherwise                     = unsafePerformIO $ do
      ...
      withFlat (fftSubgroupGen sg_src) $ \ptr_sg1 -> do
        withFlat (fftSubgroupGen sg_tgt) $ \ptr_sg2 -> do
          withForeignPtr fptr2 $ \ptr2 -> do
            withForeignPtr fptr3 $ \ptr3 -> do
              c_bls12_381_poly_mont_ntt_forward_asymmetric
