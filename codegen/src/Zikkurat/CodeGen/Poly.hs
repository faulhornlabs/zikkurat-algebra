...
c_header (PolyParams{..}) =
  ...
  , ""
  , "extern void " ++ prefix ++ "ntt_forward_asymmetric(int m_src, int m_tgt, const uint64_t *gen_src, const uint64_t *gen_tgt, const uint64_t *src, uint64_t *tgt);"
  ]
...
cForwardNTT (PolyParams{..}) = 
  ...
  , "void " ++ prefix ++ "ntt_forward_asymmetric(int m_src, int m_tgt, const uint64_t *gen_src, const uint64_t *gen_tgt, const uint64_t *src, uint64_t *tgt) {"
  , "  assert( m_tgt >= m_src );"
  , "  int N_src = (1 << m_src);"
  , "  int N_tgt = (1 << m_tgt);"
  ...
  , "  uint64_t x[NLIMBS];"
  , "  " ++ prefix_r ++ "set_one( gpows );"
  , "  " ++ prefix_r ++ "copy( gen_src, gpows + NLIMBS );"
  , "  " ++ prefix_r ++ "copy( gen_src, x );"
  , "  for(int i=2; i<halfN_src; i++) {"
  , "    " ++ prefix_r ++ "mul_inplace(x, gen_src);"
  , "    " ++ prefix_r ++ "copy( x , gpows + (i*NLIMBS) );"
  , "  }"
  , "  "
  ...
  , "    }"
  , "    else {"
  , "      " ++ prefix_r ++ "mul_inplace( eta , gen_tgt );"
  , "      "
  , "      uint64_t x[NLIMBS];"
  , "      " ++ prefix_r ++ "set_one( x );"
  ...
  , "    }"
  , "    "
  , "    " ++ prefix ++ "ntt_forward_noalloc( m_src, 1, gpows, shifted, buf, tgt_small);"
  , "    "
...
hsNTT (PolyParams{..}) =
  ...
  , ""
  , "foreign import ccall unsafe \"" ++ prefix ++ "ntt_forward_asymmetric\" c_" ++ prefix ++ "ntt_forward_asymmetric :: CInt -> CInt -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> Ptr Word64 -> IO ()"
  , ""
  , "{-# NOINLINE forwardNTT #-}"
  ...
  , "asymmForwardNTT :: FFTSubgroup " ++ typeName_r ++ " -> " ++ typeName ++ " -> FFTSubgroup " ++ typeName_r ++ " -> FlatArray " ++ typeName_r 
  , "asymmForwardNTT sg_src (Mk" ++ typeName ++ " (MkFlatArray n fptr2)) sg_tgt" 
