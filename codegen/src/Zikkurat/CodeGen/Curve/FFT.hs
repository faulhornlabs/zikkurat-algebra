...
fft_hs_binding (CodeGenParams{..}) =
  ...
  , "            c_" ++ prefix ++ "fft_forward (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3"
  ...
  , "        withForeignPtr fptr2 $ \\ptr2 -> do"
  , "          withForeignPtr fptr3 $ \\ptr3 -> do"
  , "            c_" ++ prefix ++ "fft_inverse (fromIntegral $ M.fromLog2 $ fftSubgroupLogSize sg) ptr1 ptr2 ptr3"
  , "      return (MkFlatArray n fptr3)"
  , ""
  ]
