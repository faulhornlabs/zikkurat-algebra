...
c_header (Params{..}) =
  ...
  , "extern void " ++ prefix ++ "batch_from_std ( int N, const uint64_t *src ,       uint64_t *tgt );"
  , "extern void " ++ prefix ++ "batch_to_std   ( int N, const uint64_t *src ,       uint64_t *tgt );"
  , ""
...
hsBegin params@(Params{..}) =
  ...
  , "  , batchToStd" ++ postfix ++ " , batchFromStd" ++ postfix
  , "    -- * Field elements"
  ...
  , "  ringName _ = \"" ++ fieldName ++ " (Montgomery repr.)\""
  , "  ringSize _ = prime"
  ...
  , "----------------------------------------"
  , ""
  , "foreign import ccall unsafe \"" ++ prefix ++ "batch_from_std\" c_" ++ prefix ++ "batch_from_std :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()"
  , ""
  , "{-# NOINLINE batchFromStd" ++ postfix ++ "#-}"
  , "batchFromStd" ++ postfix ++ " :: L.FlatArray (Std." ++ typeName ++ ") -> L.FlatArray (" ++ typeName ++ ")"
  , "batchFromStd" ++ postfix ++ " (MkFlatArray n fptr1) = unsafePerformIO $ do"
  , "  fptr2 <- mallocForeignPtrArray (n*" ++ show nlimbs ++ ")"
  , "  withForeignPtr fptr1 $ \\ptr1 -> do"
  , "    withForeignPtr fptr2 $ \\ptr2 -> do"
  , "      c_" ++ prefix ++ "batch_from_std (fromIntegral n) ptr1 ptr2"
  , "  return (MkFlatArray n fptr2)"
  , ""
  , "foreign import ccall unsafe \"" ++ prefix ++ "batch_to_std\" c_" ++ prefix ++ "batch_to_std :: CInt -> Ptr Word64 -> Ptr Word64 -> IO ()"
  , ""
  , "{-# NOINLINE batchToStd" ++ postfix ++ "#-}"
  , "batchToStd" ++ postfix ++ " :: L.FlatArray (" ++ typeName ++ ") -> L.FlatArray (Std." ++ typeName ++ ")"
  , "batchToStd" ++ postfix ++ " (MkFlatArray n fptr1) = unsafePerformIO $ do"
  , "  fptr2 <- mallocForeignPtrArray (n*" ++ show nlimbs ++ ")"
  , "  withForeignPtr fptr1 $ \\ptr1 -> do"
  , "    withForeignPtr fptr2 $ \\ptr2 -> do"
  , "      c_" ++ prefix ++ "batch_to_std (fromIntegral n) ptr1 ptr2"
  , "  return (MkFlatArray n fptr2)"
  , ""
  ] ++
...
montConvert Params{..} =
  ...
  , "void " ++ prefix ++ "batch_from_std( int N, const uint64_t *src, uint64_t *tgt) {"
  ...
  , "void " ++ prefix ++ "batch_to_std( int N, const uint64_t *src, uint64_t *tgt) {"
  , "  const uint64_t *p = src;"
  , "  uint64_t       *q = tgt;"
  , "  for(int i=0; i<N; i++) {"
  , "    " ++ prefix ++ "to_std(p,q);"
